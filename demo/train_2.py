...
if __name__ == '__main__':
    ...
    evaluator = FLIREvaluator(dataset, cfg, False, output_dir=out_folder, save_eval=True, out_eval_path='out/mAP/FLIR_learned_l1_w_o_bias.out')
    ...
    for i in range(loop_time):
        ...
        # 100. / train_samples
        predictor = LogisticRegression(C=50, penalty=penalty, solver='saga',verbose=1, tol=0.0001, max_iter=200, fit_intercept=use_bias, class_weight=class_weight)
        
        X_train /= np.linalg.norm(X_train+0.001, axis=1,keepdims=True)
        #X_train[:,:4] /= np.linalg.norm(X_train[:,:4]+0.001, axis=1,keepdims=True)
        #X_train[:,4:] /= np.linalg.norm(X_train[:,4:]+0.001, axis=1,keepdims=True)
    ...
    result = evaluate(cfg, evaluator, val_1, val_2, anno_val_gt, clf, method)
    pdb.set_trace()
    """