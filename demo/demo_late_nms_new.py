...
def apply_late_fusion_and_evaluate(evaluator, det_1, det_2, method):
    evaluator.reset()
    ...
    for i in range(num_img):
        ...
        if len(info_1['bbox']) == 0 or len(info_2['bbox']) == 0:
            ...
            if method == 'nms':            
                ...
                out_boxes = Boxes(in_boxes)
                out_scores = torch.Tensor(in_scores)
                out_class = torch.Tensor(in_class)
                #pdb.set_trace()
            elif method == 'baysian':
                threshold = 0.5
                fuse_method = 'baysian' #'avg_box'
                in_boxes, in_scores, in_class = prepare_data(info_1, info_2)
                keep, out_scores, out_boxes, out_class = nms_2(in_boxes, in_scores, in_class, threshold, fuse_method)
...
if __name__ == '__main__':
    ...
    dataset = 'FLIR'
    IOU = 50
    det_file_1 = data_folder + 'val_thermal_only_predictions_IOU50_new.json'
    det_file_2 = data_folder + 'val_mid_fusion_predictions_IOU50_new.json'
    ...
    method = 'baysian' #'baysian'#'nms'
    result = apply_late_fusion_and_evaluate(evaluator, det_1, det_2, method)