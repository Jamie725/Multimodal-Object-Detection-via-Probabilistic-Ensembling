...
def train_late_fusion(det_1, det_2, anno, det_3=''):
    img_folder = '../../../Datasets/FLIR/val/thermal_8_bit/'
    num_img = len(det_2['image'])
    count_1=0
    count_2=0
    ...
    for i in range(num_img):
        ...
        if len(info_1['bbox']) > 0: num_1 = 1
        else: num_1 = 0
        if len(info_2['bbox']) > 0: num_2 = 1
        else: num_2 = 0
        if len(info_3['bbox']) > 0: num_3 = 1
        else: num_3 = 0
        ...
        if img_id in anno.keys():
            ...
            # No detections
            if num_detections == 0:
            #if len(info_1['bbox']) == 0 and len(info_2['bbox']) == 0 and len(info_3['bbox']) == 0:
                continue
            # If no detection in 1st model:
            if num_detections == 1:
                if len(info_1['bbox']) > 0:
                    # print('Only model 1 get detection')
                    in_boxes, in_scores, in_class, in_logits, in_prob, num_det = prepare_data_gt_1_det(info_1, info_gt)
                    score_results, class_results, out_bboxs = nms_multiple_box(in_boxes, in_scores, in_class, in_logits, 0.5, num_det)
                elif len(info_2['bbox']) > 0:
                    #print('Only model 2 get detection')
                    in_boxes, in_scores, in_class, in_logits, in_prob, num_det = prepare_data_gt_1_det(info_2, info_gt)
                    score_results, class_results, out_bboxs = nms_multiple_box(in_boxes, in_scores, in_class, in_logits, 0.5, num_det)
                elif len(info_3['bbox']) > 0:
                    #print('Only model 3 get detection')
                    in_boxes, in_scores, in_class, in_logits, in_prob, num_det = prepare_data_gt_1_det(info_3, info_gt)
                    score_results, class_results, out_bboxs = nms_multiple_box(in_boxes, in_scores, in_class, in_logits, 0.5, num_det)
...
if __name__ == '__main__':
    ...
    X_train, Y_train = train_late_fusion(det_1, det_2, anno_train_gt, det_3=det_3)
    ...
    save_file_name = 'learning_late_fusion_weight_fuse_2_models_l2.npz'
    max_score = 0
    loop_time = 10
    for i in range(loop_time):
        predictor = LogisticRegression(C=50. / train_samples, penalty='l2', solver='saga', tol=0.1, max_iter=300)
