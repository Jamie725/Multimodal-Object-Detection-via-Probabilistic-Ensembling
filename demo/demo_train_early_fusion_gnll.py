...
out_folder = '0305/output_early_fusion_gnll'
...
cfg.MODEL.WEIGHTS = 'good_model/3_class/early_fusion/out_model_early_fusion_gnll.pth'#"detectron2://COCO-Detection/faster_rcnn_R_101_FPN_3x/137851257/model_final_f6e8b1.pkl"
cfg.MODEL.ROI_HEADS.NUM_CLASSES = 3
...
for param in trainer.model.backbone.parameters():
    param.requires_grad = False
for param in trainer.model.proposal_generator.parameters():
    param.requires_grad = False
for param in trainer.model.roi_heads.box_pooler.parameters():
    param.requires_grad = False
"""
best_AP50 = 0
for idx in range(num_loops):
    ...
    if idx > 0:
        ...
        #    param.requires_grad = False
        trainer.resume_or_load(resume=False)        
        out_model_path = os.path.join(out_folder, 'out_model_early_fusion_gnll.pth')
    ...
    if AP50 > best_AP50:
        best_AP50 = AP50
        out_model_path = os.path.join(out_folder, 'out_model_early_fusion_best.pth')
        torch.save(trainer.model.state_dict(), out_model_path)
    del trainer