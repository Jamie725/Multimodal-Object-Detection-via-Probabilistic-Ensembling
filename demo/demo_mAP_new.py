...
def test_during_train(trainer, dataset_name, out_eval_name=None):
    cfg.DATASETS.TEST = (dataset_name, )
    if out_eval_name:
        evaluator_FLIR = FLIREvaluator(dataset_name, cfg, False, output_dir=out_folder, out_pr_name='pr_val.png', out_eval_path=out_eval_name)
...
#'output_4_channel/good_model/out_model_iter_32000.pth'

trainer = DefaultTrainer(cfg)
trainer.resume_or_load(resume=False)
pdb.set_trace()
# Evaluation on validation set
test_during_train(trainer, dataset_train, out_eval_name='out/mAP/RGB_train.out')
test_during_train(trainer, dataset_test, out_eval_name='out/mAP/RGB_val.out')
del trainer
